...
async def git_push(request: GitPushRequest) -> GitPushResponse:
    """Push commits to a remote.

    Args:
        request: Push request with remote and optional branch
    ...
    try:
        ...
        stdout, stderr, code = await _run_git(args, cwd)
        if code != 0:
            return GitPushResponse(success=False, message=stderr or stdout)

        return GitPushResponse(success=True, message=stdout.strip())
    except Exception as e:
        logger.error(f"Failed to push: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/git/pr", response_model=GitPRResponse)
async def git_create_pr(request: GitPRRequest) -> GitPRResponse:
    """Create a GitHub Pull Request.

    Args:
        request: PR request with title, body, and base branch
    ...
    try:
        ...
        # Create PR
        args = ["pr", "create", "--title", request.title, "--base", request.base]
        if request.body:
        ...
        if proc.returncode != 0:
            return GitPRResponse(
                ...
                or "Failed to create PR",
            )

        url = stdout.decode("utf-8", errors="replace").strip()
        return GitPRResponse(success=True, url=url, message="PR created successfully")
    except Exception as e:
