...
export class CodeApplication extends Disposable {
	...
	private registerListeners(): void {
		...
		//#region AI Agent ACP Handlers

		validatedIpcMain.handle(
			"aiAgent:initialize",
			async (_event, params: { cwd: string }) => {
				const { acpInitialize } =
					await import("../../workbench/contrib/aiAgentPane/electron-browser/acpHost.js");
				return acpInitialize(params.cwd);
			},
		);

		validatedIpcMain.handle(
			"aiAgent:sendRequest",
			async (_event, params: { method: string; acpParams: unknown }) => {
				const { acpSendRequest } =
					await import("../../workbench/contrib/aiAgentPane/electron-browser/acpHost.js");
				try {
					const result = await acpSendRequest(params.method, params.acpParams);
		...
		validatedIpcMain.handle(
			...
			async (_event, params: { sessionId: string; text: string }) => {
				const { acpSendStreamingPrompt } =
					await import("../../workbench/contrib/aiAgentPane/electron-browser/acpHost.js");
				const stream = acpSendStreamingPrompt(params.sessionId, params.text);
				stream.onUpdate((update) => {
					// Forward to all windows
					const { BrowserWindow } = require("electron");
					for (const win of BrowserWindow.getAllWindows()) {
						if (!win.isDestroyed()) {
							win.webContents.send("aiAgent:streamUpdate", {
				...
				stream.onDone((result) => {
					...
					for (const win of BrowserWindow.getAllWindows()) {
						if (!win.isDestroyed()) {
							win.webContents.send("aiAgent:streamDone", {
								requestId: stream.requestId,
								result,
							});
						}
				...
				return { success: true, requestId: stream.requestId };
			},
		);

		validatedIpcMain.handle("aiAgent:dispose", async () => {
			const { acpDispose } =
				await import("../../workbench/contrib/aiAgentPane/electron-browser/acpHost.js");
			acpDispose();
			return { success: true };
