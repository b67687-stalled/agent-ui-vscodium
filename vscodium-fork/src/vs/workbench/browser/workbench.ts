...
import "../../contrib/aiInlinePrompt/browser/aiInlinePrompt.contribution.js";
import "./style.js";
...
export class Workbench extends Layout {
	...
	startup(): IInstantiationService {
		try {
			...
			instantiationService.invokeFunction((accessor) => {
				...
				// Layout
				// AI Editor Fork: Hello World injection (visible marker)
				const helloEl = document.createElement("div");
				this.initLayout(accessor);
				helloEl.textContent = "Hello World from AI Editor!";
				helloEl.style.cssText =
					"position:fixed;top:0;left:0;z-index:9999;background:var(--vscode-editorWidget-background,#1e1e1e);color:var(--vscode-editor-foreground,#ccc);padding:8px 16px;font-size:14px;border-bottom:1px solid var(--vscode-editorWidget-border,#333)";
				document.body.appendChild(helloEl);

				// AI Editor Fork: Load Agent Panel contribution (Phase 2)
				import("../../contrib/aiAgentPane/browser/aiAgentPane.contribution.js");
	...
	private updateFontFamily(configurationService: IConfigurationService) {
		let family = configurationService.getValue<string>(
	...
	private updateFontSize(configurationService: IConfigurationService) {
		const configuredSize = getFontSize(
			configurationService,
